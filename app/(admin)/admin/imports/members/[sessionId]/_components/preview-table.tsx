      ...
      <div className="rounded-xl border bg-card p-8 space-y-4 text-center">
        ...
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            <strong>{result.committedCount}</strong> member{result.committedCount !== 1 ? 's' : ''} created ·{' '}
            <strong>{result.skippedCount}</strong> skipped
          </p>
        </div>
        <p className="text-xs text-muted-foreground">
          Welcome emails have been queued. Check{' '}
          <Link href="/admin/email-log" className="underline">
            Email Log
          </Link>{' '}
          for delivery status.
        </p>
        <div className="flex justify-center gap-3 pt-2">
          <Link href="/admin/accounts">
            <Button variant="default">View Accounts</Button>
          </Link>
          <Link href="/admin/imports/members">
          ...
          <tbody>
            {filteredRows.map((row) => (
              ...
              >
                ...
                <td className="px-3 py-2 font-medium">{row.rowData.full_name || '—'}</td>
                <td className="px-3 py-2 text-muted-foreground">{row.rowData.email || '—'}</td>
                <td className="px-3 py-2 text-muted-foreground">{row.rowData.dob || '—'}</td>
                <td className="px-3 py-2 text-muted-foreground">{row.rowData.gender || '—'}</td>
                <td className="px-3 py-2 text-muted-foreground">{row.rowData.phone_e164 || '—'}</td>
