        ...
        <div className="rounded-xl border bg-card p-6 space-y-3">
          ...
          {session.status === 'COMMITTED' && (
            <p className="text-sm text-muted-foreground">
              {session.committedCount} member{session.committedCount !== 1 ? 's' : ''} created ·{' '}
              {session.skippedCount} skipped due to errors.
            </p>
          )}
          <Link href="/admin/imports/members" className="text-sm text-primary hover:underline">
            ← Start a new import
          </Link>
        </div>
      ) : (
