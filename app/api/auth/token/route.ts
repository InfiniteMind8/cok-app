...
function checkDemoMintLimit(ip: string): { ok: boolean; reset: number } {
  const now = Date.now()
...
// Dev/demo helper — returns a Clerk sign-in token for a demo account.
// 404s in production. No DB touches; audit trail of demo token issuance is
// the responsibility of the backend (it watches AuditLog upstream).
