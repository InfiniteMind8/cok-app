...
function normalizeNonNegativeInt(
  ...
  const n = parseFloat(s)
  if (isNaN(n)) return { value: '', error: `${fieldName} "${s}" is not a valid number`, warning: null }
...
export async function parsePropertiesSheet(
  ...
  for (let i = 0; i < raw.length; i++) {
    ...
    // status enum validation (optional field, default VACANT)
    if (rowData.status && !PROPERTY_STATUS_VALUES.includes(rowData.status as typeof PROPERTY_STATUS_VALUES[number])) {
      status = 'ERROR'
    ...
    // Duplicate external_ref within the file
    ...
    // Cross-DB duplicate: external_ref matches existing property code
