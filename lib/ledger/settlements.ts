...
function settledResultFromRequest(request: { amount: Prisma.Decimal | number | string; transactionId: string | null }) {
...
export async function executeSettlement(args: ExecuteSettlementArgs) {
  ...
  if (request.status !== 'APPROVED') {
    throw new Error(
      `Settlement ${args.settlementId} is not APPROVED; Cannot execute settlement in status ${request.status}`
    )
  }
