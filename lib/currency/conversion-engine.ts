...
export async function convertFiatToKcrd(params: {
  ...
  const bonusKcrd = promotion
    ? baseKcrd.mul(new Prisma.Decimal(promotion.bonusPercent)).div(HUNDRED).toDecimalPlaces(8, Prisma.Decimal.ROUND_HALF_EVEN)
  ...
  await db.$transaction(async (tx) => {
    ...
    await tx.auditLog.create({
      data: {
        ...
        entityId: userId,
        actorId: recordedBy,
        after: { fiatAmount: fiatAmount.toFixed(2), fiatCurrency, baseKcrd: baseKcrd.toFixed(8), bonusKcrd: bonusKcrd.toFixed(8), promotionId: promotion?.id ?? null },
      },
    })
  })
...
export async function convertKcrdToFiat(params: {
  ...
  const bonusKcrd = promotion
    ? kcrdAmount.mul(new Prisma.Decimal(promotion.bonusPercent)).div(HUNDRED).toDecimalPlaces(8, Prisma.Decimal.ROUND_HALF_EVEN)
    : new Prisma.Decimal(0)
  ...
  await db.$transaction(async (tx) => {
    ...
    await tx.auditLog.create({
      data: {
        ...
        entityId: userId,
        actorId: recordedBy,
        after: { kcrdAmount: kcrdAmount.toFixed(8), targetCurrency, bonusKcrd: bonusKcrd.toFixed(8), promotionId: promotion?.id ?? null },
      },
    })
  })
