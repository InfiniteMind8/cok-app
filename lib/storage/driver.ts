...
export class LocalStorageDriver implements StorageDriver {
  ...
  // Used by the serve endpoint to decrypt a file by key
  ...
  // Used by the serve endpoint to verify a token
...
export function getStorage(): StorageDriver {
  ...
  if (driver === 's3') {
    ...
    for (const k of required) {
      if (!process.env[k]) throw new Error(`Missing env var: ${k} (required for STORAGE_DRIVER=s3)`)
    }
